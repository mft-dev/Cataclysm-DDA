                                  ...
                                  const int range )
{
    ...
    for( size_t i = 0; i < before_hp.size(); ++i ) {
        CAPTURE( i );
        INFO( target_vehicle->get_part( i ).name() );
        if( target_vehicle->get_part( i ).info().get_id() == vpart_battery_car ||
            target_vehicle->get_part( i ).info().get_id() == vpart_headlight ||
            target_vehicle->get_part( i ).info().get_id() == vpart_windshield ) {
            CHECK( before_hp[ i ] >= after_hp[ i ] );
        } else if( !( target_vehicle->get_part( i ).info().get_id() == vpart_vehicle_clock ) ) {
            CHECK( before_hp[ i ] == after_hp[ i ] );
        }
    }
}
