...
static bool vehicle_activity( player &p, const tripoint &src_loc, int vpindex, char type )
{
    ...
    int time_to_take = 0;
    if( vpindex >= static_cast<int>( veh->part_count() ) ) {
    ...
    if( type == 'r' ) {
        const vehicle_part part = veh->get_part( vpindex );
    ...
    p.activity.values.push_back( -point_zero.y );
    // values[6]
    p.activity.values.push_back( veh->index_of_part( &veh->get_part( vpindex ) ) );
