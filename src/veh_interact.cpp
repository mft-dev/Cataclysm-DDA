...
task_reason veh_interact::cant_do( char mode )
{
    ...
    switch( mode ) {
        ...
        case 'm': {
            ...
            valid_target = std::any_of( vpr.begin(), vpr.end(), [toggling]( const vpart_reference & ref ) {
                if( toggling ) {
                    return ref.part().is_available() && !ref.part().faults_potential().empty();
                } else {
                    return ref.part().is_available() && !ref.part().faults().empty();
                }
            } );
            enough_light = g->u.fine_detail_vision_mod() <= 4;
            // checked later
