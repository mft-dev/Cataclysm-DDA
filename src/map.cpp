...
void map::add_vehicle_to_cache( vehicle *veh )
{
    ...
    int partid = 0;
    for( vpart_reference vr : veh->get_all_parts() ) {
        if( vr.part().removed ) {
            partid++;
            continue;
        }
        const tripoint p = veh->global_part_pos3( vr.part() );
    ...
    do {
        ...
        for( const auto &coll : collisions ) {
            ...
            if( static_cast<size_t>( coll.part ) > veh.part_count() ||
                veh.get_part( coll.part ).removed ) {
                continue;
            }

            const point &collision_point = veh.get_part( coll.part ).mount;
    ...
    if( !vertical && can_move ) {
        ...
        for( auto &w : wheel_indices ) {
            ...
            if( !has_flag( "SEALED", wheel_p ) ) {
                const float wheel_area =  veh.get_part( w ).wheel_area();
                                      ...
                                      const std::vector<veh_collision> &collisions )
{
    ...
    for( const auto &veh_veh_coll : collisions ) {
        ...
        epicenter1 += veh.get_part( parm1 ).mount;
        veh.damage( parm1, dmg1_part, DT_BASH );

        epicenter2 += veh2.get_part( parm2 ).mount;
...
bool map::displace_vehicle( vehicle &veh, const tripoint &dp )
{
    ...
    for( size_t i = 0; !complete && i < riders.size(); i++ ) {
        ...
        for( rider_data &r : riders ) {
            ...
            if( psg == nullptr ) {
                ...
                veh.get_part( prt ).remove_flag( vehicle_part::passenger_flag );
                r.moved = true;
                continue;
            }
            ...
            const vehicle_part &veh_part = veh.get_part( prt );
    ...
    for( vpart_reference prt : veh.get_all_parts() ) {
        prt.part().precalc[0] = prt.part().precalc[1];
    }
...
bool map::mop_spills( const tripoint &p )
{
    ...
    if( const optional_vpart_position vp = veh_at( p ) ) {
        ...
        for( auto &elem : parts_here ) {
            if( veh->get_part( elem ).blood > 0 ) {
                veh->get_part_mutable( elem ).blood = 0;
                retval = true;
            }
...
static void process_vehicle_items( vehicle &cur_veh, int part )
{
    ...
    if( washmachine_here || dishwasher_here ) {
        for( auto &n : cur_veh.get_items( part ) ) {
            ...
            if( time_left <= 0_turns ) {
                ...
                cur_veh.get_part_mutable( part ).enabled = false;
    ...
    if( recharge_part_idx >= 0 && ( recharge_part = cur_veh.get_part( recharge_part_idx ) ) &&
...
void map::add_splatter( const field_type_id &type, const tripoint &where, int intensity )
{
    ...
    if( type.obj().is_splattering ) {
        if( const optional_vpart_position vp = veh_at( where ) ) {
            ...
            if( part != -1 ) {
                veh->get_part_mutable( part ).blood += 200 * std::min( intensity, 3 ) / 3;
                return;
            }
        }
    }
    ...
    for( vehicle *v : ch.vehicle_list ) {
        for( const vpart_reference &vp : v->get_all_parts() ) {
            ...
            if( vehicle_is_opaque ) {
                ...
                if( dpart < 0 || !v->get_part( dpart ).open ) {
