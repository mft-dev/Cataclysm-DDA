...
bool basecamp::start_garage_chop( const point &dir, const tripoint &omt_tgt )
{
    ...
    while( car->part_count() > 0 ) {
        ...
        bool broken = car->get_part( prt ).is_broken();
        bool skill_break = false;
        bool skill_destroy = false;

        int dice = rng( 1, 20 );
        dice += skillLevel - car->get_part( prt ).info().difficulty;

        if( dice >= 20 ) {
            skill_break = false;
            skill_destroy = false;
            talk_function::companion_skill_trainer( *comp, skill_mechanics, 1_hours,
                                                    car->get_part( prt ).info().difficulty );
        } else if( dice > 15 ) {
            skill_break = false;
        } else if( dice > 9 ) {
            skill_break = true;
        ...
        if( !broken && !skill_break ) {
            ...
            if( !car->get_part( prt ).is_battery() ) {
                car->get_part( prt ).ammo_consume( car->get_part( prt ).ammo_remaining(),
                                         car->global_part_pos3( car->get_part( prt ) ) );
            }
            comp->companion_mission_inv.add_item( car->get_part( prt ).properties_to_item() );
        } else if( !skill_destroy ) {
            for( const item &itm : car->get_part( prt ).pieces_for_broken_part() ) {
...
std::string camp_car_description( vehicle *car )
{
    ...
    for( auto &pt : car->get_all_parts() ) {
        if( pt.part().is_battery() ) {
            const vpart_info &vp = pt.info();
            entry += string_format( ">%s:%*d%%\n", vp.name(), 32 - utf8_width( vp.name() ),
                                    static_cast<int>( 100.0 * pt.part().ammo_remaining() /
                                            pt.part().ammo_capacity( ammotype( "battery" ) ) ) );
        }
    }
    entry += "\n";
