...
class item_location::impl::item_on_vehicle : public item_location::impl
{
    ...
    public:
        ...
        void serialize( JsonOut &js ) const override {
            ...
            item tmp = cur.veh.get_part( cur.part ).base;
            if( target() != &tmp ) {
                js.member( "idx", find_index( cur, target() ) );
            }
            js.end_object();
        ...
        item *unpack( int idx ) const override {
            item tmp = cur.veh.get_part( cur.part ).base;
            return idx >= 0 ? retrieve_index( cur, idx ) : &tmp;
        }
        ...
        void remove_item() override {
            item &base = cur.veh.get_part( cur.part ).base;
            if( &base == target() ) {
...
void item_location::deserialize( JsonIn &js )
{
    ...
    if( type == "character" ) {
        ...
        if( veh && part >= 0 && part < static_cast<int>( veh->part_count() ) ) {
