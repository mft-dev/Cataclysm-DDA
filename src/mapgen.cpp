    ...
    for( std::vector<int>::const_iterator part = frame_indices.begin();
        ...
        if( other_veh != nullptr && other_veh->type != vproto_id( "shopping_cart" ) ) {
            ...
            for( const vpart_reference &ref : veh->get_all_parts() ) {
                const tripoint part_pos = veh->global_part_pos3( ref.part() ) - global_pos;
                // TODO: change mount points to be tripoint
                wreckage->install_part( part_pos.xy(), ref.part() );
            }

            for( const vpart_reference &ref : other_veh->get_all_parts() ) {
                const tripoint part_pos = other_veh->global_part_pos3( ref.part() ) - global_pos;
                wreckage->install_part( part_pos.xy(), ref.part() );
